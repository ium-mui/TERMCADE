...
플레이 결과, 전체 이력, 지갑 잔액은 운영체제별 앱 데이터 디렉터리의 JSON 파일에 저장됩니다. 런처형 TUI 쉘(상단 상태 바·좌측 내비게이션·하단 컨트롤 바) 안에서 카드형 선택 화면과 보드·상태 패널을 함께 표시하며, 터미널이 너무 작으면 화면이 잘릴 수 있으므로 최소 `76×22` 크기를 권장합니다.

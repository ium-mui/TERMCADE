...
- **Documentation issue**：缺失、错误或过时的内容
...
支持的类型为 `feat`、`fix`、`docs`、`refactor`、`test`、`perf`、`build`、`ci`、`chore` 和 `revert`。发布工具读取的类型和可选 scope 保持 ASCII，摘要应清晰简洁。
...
- 说明目的、重要实现选择和验证方式。
- 为修改的行为添加测试，并更新相关用户文档。
- 在本地运行 `./scripts/check.sh`。
- 解决审阅对话；`main` 更新后同步分支。

代码标识符和命令应保持准确，便于验证修改。

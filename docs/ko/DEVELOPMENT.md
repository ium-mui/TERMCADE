...
이 스크립트는 CI와 동일한 문서 검사, 포맷, Clippy, 테스트를 수행합니다. 개발 중에는 필요한 명령만 실행할 수 있습니다.
...
8. 게임 문서를 갱신하고 `./scripts/check.sh`를 실행합니다.
...
문서가 현재 코드와 사용자에게 보이는 동작을 정확히 설명하도록 유지합니다. 가능하면 관련 문서 변경을 같은 PR에 포함합니다.
...
- 사용자 동작과 아키텍처 문서가 필요한 부분에서 갱신됐습니다.

...
8. 영어 게임 문서와 두 번역을 갱신하고 `./scripts/check.sh`를 실행합니다.
...
영어가 기준입니다. 가능하면 같은 PR에서 `docs/ko`와 `docs/zh-CN` 파일도 갱신합니다. [번역 정책](TRANSLATIONS.md)을 따르며 CI는 모든 기준 문서에 두 로케일 파일이 있는지 확인합니다.
